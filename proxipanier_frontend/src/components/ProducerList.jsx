    ...
    return (
        <div className="flex flex-col items-center mt-10">
            <div className="mb-2 w-full max-w-md">
                <label htmlFor="distanceRange" className="text-[#FBD784] mb-1 block text-sm">Distance de recherche</label>
                <input
                    id="distanceRange"
                    type="range"
                    min={0}
                ...
                {filteredProducers.length > 0 ? (
                    filteredProducers.map((user, index) => {
                        ...
                        return (
                            <div
                                key={user.id}
                                className={`flex ${index % 2 === 0 ? 'flex-row' : 'flex-row-reverse'} bg-transparent w-full rounded-lg shadow-lg p-2 border-2 border-[#FBD784] relative`}
                            >
                                <div className="card-body">
                                    <h3 className="card-title text-[#FBD784] text-base">{user.name}</h3>
                                    <p className="text-sm text-white">Ville: {user.city || 'Non spécifiée'}</p>
                                    <p className="text-sm text-white">
                                        Ventes actuelles: {salesNames}
                                    </p>
                                    <div className="card-actions justify-end items-center">
                                        <Link to={`/producer/${user.id}`} className="btn text-xs hover:text-[#0e2631] text-[#FBD784] hover:bg-[#FBD784] bg-transparent border-[#FBD784] hover:border-none">
                                            Voir plus
                                        </Link>
                                        {id !== user.id && role !== 2 ? (
                                            <div className="absolute top-2 right-2">
                                                <FontAwesomeIcon
                                                    ...
                                                    className="h-4 w-4 cursor-pointer text-[#FBD784]"
                                                />
                                            </div>
                                        ) : null}
                                    </div>
                    ...
                    <p className="text-sm text-white">Aucun producteur disponible dans un rayon de {distanceLimit} km.</p>
                )}
            </div>
        </div>
    );
